...
function initDemo() {
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const slider2 = jQuery('.second_wrapper>div:last-child').rangeSlider({
      type: 'point', origin: 10, range: 90, step: 5, scaleInterval: 20,
    });
    slider2.init(30);
    ...
    const slider3 = jQuery('.third_wrapper>div:last-child').rangeSlider({
      type: 'point', orient: 'vertical', origin: 0, scaleInterval: 5, range: 10,
    });
    slider3.init(5);
    connectThePanel(block3, slider3);

    const block4 = jQuery('.fourt_wrapper>.panel');
    const slider4 = jQuery('.fourt_wrapper>div:last-child').rangeSlider({ type: 'range', list: ['ἄ', 'β', 'γ', 'λ', 'Ξ', 'ζ', 'π', 'θ', 'ψ'], cloud: 'none' });
    slider4.init(1, 3);
  ...
  function connectThePanel(panelNode: JQuery, sliderObject:sliderObjectI) {
    ...
    panelNode.find("input[name='range']").on('change', (e) => {
      e.preventDefault();
      const value = Math.max(Number((e.target as HTMLInputElement).value), 1);
    ...
    panelNode.find("input[name='second_pos']").on('keydown', (e) => {
      const { value } = e.target as HTMLInputElement;
      if (e.code === 'Enter') {
        if (slider.config.type === 'point') slider.setValue(value);
        else slider.setValue(undefined, value);
      }
    });
    panelNode.find('.values-enter-button').on('click', (e) => {
      e.preventDefault();
      const start = panelNode.find("input[name='first_pos']").val();
      const end = panelNode.find("input[name='second_pos']").val();
      if (slider.config.type === 'point') slider.setValue(end);
      else slider.setValue(start, end);
    });
